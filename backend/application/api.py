...
class UserAPI(Resource):
    ...
    def post(user, self):
        if user.role_id in [3, 4]:
            ...
            user_name = data['email_id'].split('@')[0]
            # Create user in your Flask application database
            ...
            if discourse_response.status_code == 200:
                ...
                # Rollback user creation in Flask application if Discourse user creation fails
                db.session.rollback()
                return jsonify({'error': 'Failed to create user in Discourse'}), 500
        else:
            abort(403, message="You are not authorized to view this page")        
    @token_required
    def patch(user,self):
        ...
        try:
            ...
            if(user.role_id==3):
                ...
                abort(403,message="You can't edit email")
        except:
            pass
        db.session.commit()
    ...
    def delete(user,self,user_id):
        if user.role_id==3:
            ...
            if current_user:
                ...
                #Deleting the user from discourse
                response = requests.delete(f"http://localhost:4200/admin/users/{id}.json", headers=headers)
